...
test('creates a clamped nested Ellipse through the published Core command API', () => {
  const ids = [
    '11111111-1111-4111-8111-111111111111',
    '22222222-2222-4222-8222-222222222222',
    '33333333-3333-4333-8333-333333333333',
  ...
  unwrap(controller.createEllipseAt(999, 999));
  ...
  expect(controller.snapshot().document.nodes).toHaveLength(1);
...
test('creates editable text through the published Core command API', () => {
  ...
  unwrap(controller.createFrameAt(80, 100));
  unwrap(controller.createTextAt(140, 160));

  expect(controller.snapshot()).toMatchObject({
    document: {
      revision: 2,
      nodes: [
        { type: 'frame', childIds: ['44444444-4444-4444-8444-444444444444'] },
        {
          id: '44444444-4444-4444-8444-444444444444',
          type: 'text',
          parentId: '33333333-3333-4333-8333-333333333333',
...
test('creates one detached frame-local Path with stable generated identities and history', () => {
  const ids = [
    '11111111-1111-4111-8111-111111111111',
    '22222222-2222-4222-8222-222222222222',
    '33333333-3333-4333-8333-333333333333',
    ...
    anchors: [
      {
        ...
        outgoingControl: { x: 12, y: 0 },
      },
      {
        position: { x: 180, y: 120 },
        incomingControl: { x: -12, y: 0 },
  ...
  unwrap(controller.createPath(completed));
  (completed.anchors[0].position as { x: number }).x = 999;
  ...
  expect(controller.layers()[1]).toMatchObject({ type: 'path', name: 'Path', depth: 1 });
  unwrap(controller.undo());
  expect(controller.snapshot().document.nodes).toHaveLength(1);
  unwrap(controller.redo());
  expect(controller.snapshot().document.nodes[1]?.type).toBe('path');
});

test('rejects malformed completed Path input before allocating an identity', () => {
  const ids = [
    '11111111-1111-4111-8111-111111111111',
    '22222222-2222-4222-8222-222222222222',
    '33333333-3333-4333-8333-333333333333',
  ...
  expect(
      ...
      anchors: [
        {
          position: { x: 0, y: 0 },
          incomingControl: { x: 0, y: 0 },
          outgoingControl: { x: Number.NaN, y: 0 },
        },
        {
          position: { x: 10, y: 10 },
          incomingControl: { x: 0, y: 0 },
    ...
    ok: false,
    error: { code: 'interaction.coordinate-invalid', path: '/anchors/0/outgoingControl' },
  });
  expect(controller.snapshot()).toEqual(before);
  unwrap(controller.createTextAt(10, 10));
  expect(String(controller.snapshot().document.nodes[1]?.id)).toBe(
    '44444444-4444-4444-8444-444444444444',
  );
});
